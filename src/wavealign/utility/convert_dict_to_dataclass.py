...
def convert_dict_to_dataclass(
    class_type: Type[DataClassT], input_dict: dict
) -> DataClassT:
    ...
    for key, value in input_dict.items():
        if key not in field_types:
            raise ValueError(f"Additional field '{key}' found in the data dictionary.")

        if hasattr(field_types[key], "__dataclass_fields__"):
            input_dict[key] = convert_dict_to_dataclass(field_types[key], value)

        if isinstance(value, list) and hasattr(
            field_types[key].__args__[0], "__dataclass_fields__"
        ):
            nested_cls = field_types[key].__args__[0]
            input_dict[key] = [convert_dict_to_dataclass(nested_cls, v) for v in value]
