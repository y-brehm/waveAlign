        ...
        if ((wrdpntr[0] & 0xFF) != 0xFF)
			...
			if (!LayerSet) {
				switch (wrdpntr[1] & 0x06) {
					...
					case 0x04:
						BadLayer = !0;
                        fprintf(stderr, "is an MPEG Layer II file, not a layer III file\n");
						return 0;
				}
			}
		...
		while (ok) {
			...
			if (singlechannel) {
				if ((curframe[3] >> 6) & 0x01) { /* if mode is NOT stereo or dual channel */
                    fprintf(stderr, "Can't adjust single channel for mono or joint stereo\n");
					ok = 0;
				}
			}
			if (bitridx == 0) {
    ...
    while ((opt = getopt(argc, argv, "i:g:")) != -1) {
        switch (opt) {
            case 'i':
                ...
                break;
            case 'g':
                gainChange = atof(optarg); // Convert optarg to float
                break;
            default:
