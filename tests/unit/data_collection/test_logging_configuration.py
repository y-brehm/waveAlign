...
from wavealign.data_collection.logging_configuration import (
    create_logging_config, setup_logging, output_logfile_warning
)
...
class TestLoggingConfiguration(unittest.TestCase):
    ...
    def test_setup_logging(
            self, 
            mock_create_logging_config,
            mock_config_dictConfig
    ):
        output_path = "/path/to/directory"
        setup_logging(output_path)
        mock_create_logging_config.assert_called_once_with(output_path)
        mock_config_dictConfig.assert_called_once_with(mock_create_logging_config.return_value)
    ...
    def test_output_logfile_warning(self, mock_os_path_exists, mock_os_path_join):
        ...
        mock_print.assert_called_once_with(
            ...
            f"A log file at dummy_path was created."
        )
