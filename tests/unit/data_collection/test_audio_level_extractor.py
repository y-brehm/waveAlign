...
class TestAudioLevelExtractor(unittest.TestCase):
    def setUp(self):
        ...
        self.window_size = WindowSize.LUFS_M
    ...
    def test_extract_lufs(self):
        ...
        audio_level_extractor = AudioLevelExtractor(
            gain_calculation_strategy, self.sample_rate, self.window_size
        )
        ...
        self.assertAlmostEqual(
            audio_level_extractor.extract(self.fake_audio_data),
            expected_value,
            places=6,
        )
    ...
    def test_extract_audio_level_integrated(self, mock_audio_level_calculator):
        gain_calculation_strategy = GainCalculationStrategy.LUFS
        mock_audio_level_calculator.return_value.calculate_level.return_value = 1.0
        audio_level_extractor = AudioLevelExtractor(
            gain_calculation_strategy, self.sample_rate, window_size=WindowSize.LUFS_I
        )

        result = audio_level_extractor.extract(self.fake_audio_data)

        mock_audio_level_calculator.return_value.calculate_level.assert_called_once_with(
            self.fake_audio_data
        )
        self.assertEqual(result, 1.0)

    @mock.patch(
        "wavealign.data_collection.audio_level_extractor.WindowedLevelCalculator"
    )
    @mock.patch("wavealign.data_collection.audio_level_extractor.LUFSCalculator")
    def test_extract_audio_level_with_window(
        self, mock_audio_level_calculator, mock_windowed_level_calculator
    ):
        gain_calculation_strategy = GainCalculationStrategy.LUFS

        mock_windowed_level_calculator.return_value.get_loudest_window.return_value = [
            1,
            2,
            3,
        ]
        ...
        audio_level_extractor = AudioLevelExtractor(
            gain_calculation_strategy, self.sample_rate, window_size=WindowSize.LUFS_M
        )

        result = audio_level_extractor.extract(self.fake_audio_data)

        mock_windowed_level_calculator.return_value.get_loudest_window.assert_called_once_with(
            self.fake_audio_data
        )
        mock_audio_level_calculator.return_value.calculate_level.assert_called_once_with(
            [1, 2, 3]
